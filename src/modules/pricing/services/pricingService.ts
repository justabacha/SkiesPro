...
import { PriceResponseDto, CandleRequestDto, AssetResponseDto, MarketStatusResponseDto } from '../dto/pricing.dto';
...
export class PricingService {
  ...
  async getLatestPrice(symbol: string): Promise<PriceResponseDto> {
    // Try cache first
    let tick = await this.distributionService.getLatestPrice(symbol);

    if (!tick) {
      // Fallback to DB
      const dbTick = await this.tickRepo.getLatest(symbol);
      if (!dbTick) throw new Error(`No price data available for ${symbol}`);
      tick = {
  ...
  async getCandles(query: CandleRequestDto) {
    ...
    const candles = await this.candleRepo.getCandles(query.symbol, granularity, from, to, limit);
  ...
  async getMarketStatus(symbol: string): Promise<MarketStatusResponseDto> {
    const isOpen = await this.marketStatusService.isMarketOpen(symbol);
    const hours = await this.marketStatusService.getMarketHours(symbol);
