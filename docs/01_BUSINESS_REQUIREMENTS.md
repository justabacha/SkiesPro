...
The platform simplifies retail trading into intuitive contracts (Yes/No predictions on asset price movement) with predefined risk and return parameters. It removes the necessity of external broker accounts.
...
| :--- | :--- | :--- | :--- | :--- |
| **Trading Margin (The House Edge)** | Pricing contracts so winning payouts represent 60% of the stake (leaving 40% of losing stakes as platform gross profit). | Highest potential margin; scales directly with trading volume. | Higher risk exposure if user win-rates spike unexpectedly. | **Primary Model**: Implement dynamic pricing algorithms to adjust payouts based on exposure. |
...
sequenceDiagram
    autonumber
    Trader->>Trading Engine: Place Trade (Symbol, Stake, Contract Type, Expiry)
...
| :--- | :--- | :--- |
| **Minimum Deposit** | 500 KES | Prevents micro-transaction gateway fees from consuming margins. |
| **Minimum Withdrawal** | 1,500 KES | Manages operational overhead for cash processing. |
| **Maximum Stake per Trade** | 50,000 KES | Protects capital reserves from single-trade shocks. |
| **Minimum Trade Expiry** | 1 Minute | Prevents extreme latency front-running of price ticks. |
| **Maximum Active Exposure** | 1,000,000 KES | The maximum aggregate amount the platform will risk on a single asset at any moment. |
| **Inactivity Grace Period** | 6 Months | Duration before an account is marked as inactive. |
| **Inactivity Monthly Fee** | 500 KES | Fee applied to inactive accounts with positive balances. |
| **Automatic Approval Limit** | 10,000 KES | Withdrawals below this value clear automatically, while larger requests undergo manual review. |
...
*   Payout percentages are fixed at **60%** per the Project Owner's decision (ProjectAnswers #33).
*   **Payout Equation**: If a user stakes $S$ on a contract, the settlement payouts are:
    *   **User Wins**: Payout to user = $S \times (1 + 0.60)$. Net platform expense = $- (S \times 0.60)$.
    ...
    *   At a 60% payout rate, a 50/50 win rate yields a stable **20% net platform profit margin** on total volume.
...
*   **Payout Rate**: The profit percentage paid to a user on a winning trade (e.g., 60% means a 1,000 KES winning trade returns 1,600 KES).
